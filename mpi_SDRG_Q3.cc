...
double *cr,*dr,*drsta,*load1,*load2,*drcopy,*lncr,*lndrsta,*dr2,*drcopy2,*drsta2,*dr3,*drcopy3,*drsta3;
j *bondj;
q3 *bondq3;
q2 *bondq2;
p *neighbor;
...
void writeDrr(){
        ...
        if(fp==NULL)
            ...
            for (int i=2; i<=L/2; i++ )
                fprintf(fp,"%d\t%.10f\t%.10f\t%.10f\n",i,dr3[i]/(L*N),dr1[i]/(L*N),dr2[i]/(L*N));
             fclose(fp);
        }
        ...
        if(ffp==NULL)
            ...
            for (int i=2; i<=L/2; i++ )
                fprintf(ffp,"%d\t%.10f\t%.10f\t%.10f\n",i,drsta3[i]/(L*N),drsta1[i]/(L*N),drsta2[i]/(L*N));
            fclose(ffp);
        }
...
int main(int argc, char** argv){
    ...
    r250_srandom(iseed+5555*myid);        // assign seed to random number generator
